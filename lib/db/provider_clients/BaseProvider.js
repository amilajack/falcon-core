 ...
 */
let BaseProvider = (_temp = _class = class BaseProvider {
    ...
    return _asyncToGenerator(function* () {
      ...
      try {
        ...
        if (_this.server.config.ssh && !_this.server.sshTunnel) {
          ...
          var _server$sshTunnel$add = _this.server.sshTunnel.address();

          const address = _server$sshTunnel$add.address,
                port = _server$sshTunnel$add.port;
        ...
        var _ref = yield Promise.all([driver(_this.server, _this.database), _this.handleSSHError(_this.server.sshTunnel)]),
            _ref2 = _slicedToArray(_ref, 1);

        const connection = _ref2[0];
  ...
  loadConfigLimit() {
    return _asyncToGenerator(function* () {
      if (BaseProvider.limitSelect === null) {
        var _ref3 = yield config.get();

        const limitQueryDefaultSelectTop = _ref3.limitQueryDefaultSelectTop;
  ...
  getJsonString(exportOptions) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const getSingleTable = function getSingleTable(tableName) {
...
}, _class.DEFAULT_LIMIT = 1000, _class.limitSelect = null, _temp);
