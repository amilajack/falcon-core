...
class SqliteProvider extends _BaseProvider2.default {
  ...
  /**
   * Deletes records from a table. Finds table's primary key or rowid then deletes
   * specified columns. All SQLite tables must have either a rowid or a primary key
   */
  delete(table, identifiers, isRowID = false) {
    var _this5 = this;

    return _asyncToGenerator(function* () {
      const primaryKey = yield _this5.getPrimaryKeyColumn(table);
      const conditions = isRowID ? identifiers.map(function (id) {
        return `rowid=${id}`;
  ...
  /**
   * Gets all the primary keys of a table
   */
  getDatabasePrimaryKeys() {
    var _this8 = this;

    return _asyncToGenerator(function* () {
      const tables = yield _this8.listTables();
      const keys = yield Promise.all(tables.map(function (table) {
        return Promise.all([table.name, _this8.getPrimaryKeyColumn(table.name)]).then(function (results) {
          return { tableName: results[0], primaryKey: results[1].name };
        }).catch(function (err) {
          return err;
        });
      }));
  ...
  renameTableColumns(table, columns) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const originalColumns = yield _this14.getTableColumnNames(table);
  ...
  dropTableColumns(table, columnsToDrop) {
    ...
    return _asyncToGenerator(function* () {
      const temp = yield _this15.getTableColumnNames(table);
  ...
  getCreateTableSql(table) {
    ...
    return _asyncToGenerator(function* () {
      const createTableArgs = yield _this16.getTablePropertiesSql(table);
      return `
  ...
  getTablePropertiesSql(table) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const creationScript = yield _this17.driverExecuteQuery({
        query: sql
      }).then(function (res) {
  ...
  getTableColumnNames(table) {
    ...
    return _asyncToGenerator(function* () {
      _this20.checkIsConnected();
      const columns = yield _this20.listTableColumns(table);
  ...
  listDatabases() {
    ...
    return _asyncToGenerator(function* () {
      const result = yield _this24.driverExecuteQuery({
  ...
  truncateAllTables() {
    ...
    return _asyncToGenerator(function* () {
      return _this27.runWithConnection(_asyncToGenerator(function* () {
        ...
        return _this27.driverExecuteQuery({ query: truncateAllQuery });
      }));
    })();
  }
  ...
  startGraphQLServer() {
    ...
    return _asyncToGenerator(function* () {
      if (_this28.graphQLServerIsRunning()) {
        return;
      }

      const app = (0, _express2.default)();
      const schema = yield (0, _tuql.buildSchemaFromDatabase)(_this28.connection.dbConfig.database);
      const port = yield (0, _getPort2.default)();
      app.use('/graphql', (0, _cors2.default)(), (0, _expressGraphql2.default)({ schema }));

      yield new Promise(function (resolve) {
        _this28.graphQLServer = app.listen(port, function () {
          _this28.graphQLServerPort = port;
          console.log(` > Running at http://localhost:${port}/graphql`);
          resolve();
        });
        _this28._graphQLServerIsRunning = true;
      });
    })();
  }
  ...
  stopGraphQLServer() {
    ...
    return _asyncToGenerator(function* () {
      if (_this29.graphQLServerIsRunning()) {
        _this29.graphQLServer.close();
        _this29.graphQLServer = undefined;
        _this29.graphQLServerPort = undefined;
        _this29._graphQLServerIsRunning = false;
      }
    })();
  }
