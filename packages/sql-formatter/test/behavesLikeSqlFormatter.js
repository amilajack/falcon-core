...
export default function behavesLikeSqlFormatter(language) {
  describe('foo', () => {
    ...
    it('formats SELECT with toplevel reserved words', () => {
      const result = format(
    ...
    it('keeps short parenthized list with nested parenthesis on single line', () => {
      const result = format('SELECT (a + b * (c - NOW()));');
      expect(result).toBe('SELECT\n' + '  (a + b * (c - NOW()));');
    });

    it('breaks long parenthized lists to multiple lines', () => {
      const result = format(
    ...
    it('formats postgres specific operators', () => {
