...
class Tokenizer {
  ...
  createLineCommentRegex(lineCommentTypes) {
    return new RegExp(`^((?:${lineCommentTypes.map(c => (0, _escapeRegExp2.default)(c)).join('|')}).*?(?:\n|$))`);
  }
  ...
  createParenRegex(parens) {
    return new RegExp(`^(${parens.map(p => (0, _escapeRegExp2.default)(p)).join('|')})`);
  }
  ...
  createPlaceholderRegex(types, pattern) {
    ...
    const typesRegex = types.map(_escapeRegExp2.default).join('|');
