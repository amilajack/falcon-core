import { GraphQLInt, GraphQLString, GraphQLBoolean, GraphQLFloat, GraphQLEnumType, GraphQLList } from 'graphql';
...
export function toGraphQL(sequelizeType, sequelizeTypes) {
  ...
  if (sequelizeType instanceof FLOAT || sequelizeType instanceof DOUBLE) return GraphQLFloat;
  ...
  if (sequelizeType instanceof ENUM) {
    return new GraphQLEnumType({
      name: 'tempEnumName',
      values: _(sequelizeType.values).mapKeys(sanitizeEnumValue).mapValues(v => ({ value: v })).value()
    });
  }

  if (sequelizeType instanceof VIRTUAL) {
    let returnType = sequelizeType.returnType ? toGraphQL(sequelizeType.returnType, sequelizeTypes) : GraphQLString;
    return returnType;
  }
  ...
  function sanitizeEnumValue(value) {
    return value.trim().replace(/([^_a-zA-Z0-9])/g, (_, p) => specialCharsMap.get(p) || ' ').split(' ').map((v, i) => i ? _.upperFirst(v) : v).join('').replace(/(^\d)/, '_$1');
  }
}
