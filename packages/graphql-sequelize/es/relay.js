...
import { fromGlobalId, connectionFromArray, nodeDefinitions, connectionDefinitions, connectionArgs } from 'graphql-relay';
...
export function sequelizeNodeInterface(sequelize) {
  ...
  const nodeObjects = nodeDefinitions(idFetcher(sequelize, nodeTypeMapper), typeResolver(nodeTypeMapper));
...
export function sequelizeConnection({
  ...
  let $connectionArgs = _extends({}, connectionArgs);
